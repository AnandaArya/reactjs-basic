const root = document.querySelector('#root'); // Untuk memanipulasi DOM di react kita harus mengeksekusi kodenya saat komponen itu di render, kalau tidak hasilnya akan null
// kode manipulasi DOMnya ada didalam React.useEffect

function App() {
  const [login, setLogin] = React.useState(false); // ada cara alternatifnya dari React yaitu menggunakan use.Ref(refference), kita set null dulu sebagai default valuenya

  const judulRef = React.useRef(null); // cara 1
